...
fn register_route<Service, State>(
    state: &mut State, handler: rest_api::ABBHandler<Service>, scope: actix_web::Scope, http_method: actix_web::http::Method
) -> actix_web::Scope
where
    ...
    Service: services::ABBService + 'static,
{
    ...
    scope.route("/", actix_web::web::method(http_method).to(
        async move |data: actix_web::web::Data<State>, request: actix_web::HttpRequest| {
            let found_handler = data.get_handler_map()
    ...
    State: rest_api::WithHandlerMap + 'static,
{
    pub fn new(name: &str, state: &'a mut State) -> CollectionResource<'a, State> {
        CollectionResource {state, scope: actix_web::web::scope(format!("/{}", name).as_str())}
    }

    pub fn list<Service>(
        self, handler: rest_api::ABBHandler<Service>
    ) -> CollectionResource<'a, State>
    where
        Service: services::ABBService + 'static
    {
        let scope = register_route(self.state, handler, self.scope, actix_web::http::Method::GET);
        CollectionResource { state: self.state, scope }
    }
